...
class BTree(object):
    ...
    def _build_bulkloaded_branches(self, tupla):
        leaves,seps=tupla
        minimum = self.order // 2
        levels = [leaves]
...
class BPlusTreeTests(unittest.TestCase):
    ...
    def test_bulkload(self):
        bt = BPlusTree.bulkload(list(zip(range(2000), map(str, list(range(2000))))), 20)
 
        self.assertEqual(list(bt), list(range(2000)))
 
        self.assertEqual(
                list(bt.iteritems()),
                list(zip(list(range(2000)), list(map(str, range(2000))))))
